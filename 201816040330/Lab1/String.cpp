...
const String &String::operator+(const String &a)
{
   string temp;/* Declare a temporary String variable named temp */
    ...
    (a+temp).length=temp.length;/* Set temp s length to be the sum of the two argument Strings  lengthes */
   sPtr = new char[ length + 1 ];/* Allocate memory for temp.length + 1 chars and assign the pointer to temp.sPtr */
   assert( sPtr != 0 ); // terminate if memory not allocated
  temp=a; /* Copy the left String argument s contents into temp.sPtr */
