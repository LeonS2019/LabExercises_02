...
const String &String::operator+=(const String &right )/* Write the header for the operator+ member function */
{
   ...
   temp.Length = length + right.length;/* Set temp’s length to be the sum of the two argument Strings’ lengthes */
   sPtr = new char[ length + 1 ];/* Allocate memory for temp.length + 1 chars and assign the pointer to temp.sPtr */
   assert( sPtr != 0 ); // terminate if memory not allocated
   temp.sPtr = copy.sPtr/* Copy the left String argument’s contents into temp.sPtr */
   strcat( tempPtr + length, right.sPtr );/* Write a call to strcat to concatenate the string in right
